...
#Decorator tracker emissioni CO2 (crea un file CSV sulla directory attuale con il nome di emissions.csv, potrebbe essere sovrascritta ad ogni run)
@track_emissions(project_name="SuperYOLO")
def test(data,
         weights=None,
         batch_size=32,
         imgsz=640,
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)): #zjq
        ...
        with torch.no_grad():
            ...
            t = time_synchronized()
            try:
                out, train_out = model(img,input_mode=input_mode) #zjq inference and training outputs
            except:
                out, train_out,_ = model(img,input_mode=input_mode) #zjq inference and training outputs
        ...
        if plots: #and batch_i < 3: #zjq
            ...
            #if input_mode == 'IR': #Ho commentato questa riga perchÃ© in teoria non dovrebbe mai entrare nell if, pulisco un po'
                #Thread(target=plot_images, args=(ir, targets, paths, f, names), daemon=True).start()      
            #else:
            Thread(target=plot_images, args=(img, targets, paths, f, names), daemon=True).start() #da fare tab se Ã¨ sbagliato
            ...
            #else:
            Thread(target=plot_images, args=(img, output_to_target(out), paths, f, names), daemon=True).start() #da fare tab se Ã¨ sbagliato
    # Compute statistics
    ...
    import xlsxwriter
    workbook = xlsxwriter.Workbook('workbook.xlsx') # å»ºç«‹æ–‡ä»¶
    worksheet = workbook.add_worksheet() # å»ºç«‹sheetï¼Œ å�¯ä»¥work.add_worksheet('employee')æ�¥æŒ‡å®šsheetå��ï¼Œä½†ä¸­æ–‡å��ä¼šæŠ¥UnicodeDecodeErroçš„é”™è¯¯
