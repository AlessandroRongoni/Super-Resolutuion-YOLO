...
def test(data,
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)): #zjq
        ...
        if plots: #and batch_i < 3: #zjq
            ...
            #if input_mode == 'IR': #Ho commentato questa riga perché in teoria non dovrebbe mai entrare nell if, pulisco un po'
                #Thread(target=plot_images, args=(ir, targets, paths, f, names), daemon=True).start()      
            #else:
            Thread(target=plot_images, args=(img, targets, paths, f, names), daemon=True).start() #da fare tab se è sbagliato
            ...
            #else:
            Thread(target=plot_images, args=(img, output_to_target(out), paths, f, names), daemon=True).start() #da fare tab se è sbagliato
    # Compute statistics
    ...
    import xlsxwriter
    workbook = xlsxwriter.Workbook('workbook.xlsx') # 建立文件
