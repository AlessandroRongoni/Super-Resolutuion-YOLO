...
def img2img_512_paths(img_paths): #zjq
    ...
    return [x.replace('DATASET_1280', 'DATASET') for x in img_paths] #replace('.' + x.split('.')[-1], '.txt')
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        self.stride = stride
        if hr_input== False:
            self.img_path = '/disks/disk3/CVprojects2023/group5/Progetto/SuperYOLO-detection_armi/dataset/DETECTION/images' #zjq the path for 512*512 images
        else:
            self.img_path = '/disks/disk3/CVprojects2023/group5/Progetto/SuperYOLO-detection_armi/dataset/DETECTION_1280/images' #zjq the path for 1024*1024 images
        ...
        with open(path, "r") as file:
            ...
            for j in range(len(self.img_files)):
                self.img_files[j] = self.img_path + self.img_files[j].rstrip() + '.jpg' 
        # Check cache
        self.label_files = img2label_paths(self.img_files)  # labels
        cache_path = Path(self.label_files[0]).parent.with_suffix('.cache')  # cached labels
        if cache_path.is_file():
            cache = torch.load(cache_path)  # load
            if cache['hash'] != get_hash(self.label_files + self.img_files) or 'results' not in cache:  # changed #zjq
        ...
        # Read cache
        cache.pop('hash')  # remove hash #pop() zjq popå‡½æ•°ç”¨äºŽç§»é™¤åˆ—è¡¨ä¸­çš„ä¸€ä¸ªå…ƒç´ ï¼ˆé»˜è®¤æœ€å�Žä¸€ä¸ªå…ƒç´ ï¼‰ï¼Œå¹¶ä¸”è¿”å›žè¯¥å…ƒç´ çš„å€¼
    ...
    def cache_labels(self, path=Path('./labels.cache'), prefix=''):
        ...
        pbar = tqdm(zip(self.img_files, self.label_files), desc='Scanning images', total=len(self.img_files)) #zjq
        for i, (im_file, lb_file) in enumerate(pbar): #zjq
            try:
                # verify images
        ...
        x['hash'] = get_hash(self.label_files + self.img_files) #zjq
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            # Load mosaic
            img, labels = load_mosaic(self, index) #zjq
            shapes = None

            # MixUp https://arxiv.org/pdf/1710.09412.pdf
            if random.random() < hyp['mixup']:
                img2, labels2 = load_mosaic(self, random.randint(0, self.n - 1)) #zjq
    ...
    def collate_fn(batch):
        img, label, path, shapes = zip(*batch)  # transposed  #zjq
        for i, l in enumerate(label):
            l[:, 0] = i  # add target image index for build_targets()
        return torch.stack(img, 0), torch.cat(label, 0), path, shapes #zjq
...
class LoadImagesAndLabels_sr(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        with open(path, "r") as file:
            ...
            #         self.img_files.remove(i+'\n')
            for j in range(len(self.img_files)):
                self.img_files[j] = self.img_files[j].rstrip() + '.jpg'
...
def letterbox(img, new_shape=(640, 320), color=(114, 114, 114), auto=True, scaleFill=False, scaleup=True):
