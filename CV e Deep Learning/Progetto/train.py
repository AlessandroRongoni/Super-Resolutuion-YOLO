...
# Decorator Tracker emissioni C02 (crea un file CSV sulla directory attuale con il nome di emissions.csv, potrebbe essere sovrascritta ad ogni run)
...
def train(hyp, opt, device, tb_writer=None):
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        optimizer.zero_grad()
        for i, (imgs, targets, paths, _) in pbar:  # batch zjq  -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                ...
                if opt.super:# and not opt.attention and not opt.super_attention:
                    pred,output_sr,_ = model(imgs,opt.input_mode)  # forward #zjq
                ...
                #     pred, attention_mask,_ = model(imgs,irs,opt.input_mode)
                    ...
                    pred,_ = model(imgs,opt.input_mode)
                # t1 = time.time()
                # print(t1-t0)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input_mode', type=str,default='RGB',help ='RGB IR RGB+IR(pixel-level fusion) RGB+IR+fusion(feature-level fusion)') #Ma se io modificassi gli argument per far si che parta sempre RGB?
    parser.add_argument('--batch-size', type=int, default=16, help='total batch size for all GPUs')
    parser.add_argument('--train_img_size', type=int,default=1280, help='train image sizes,if use SR,please set 1280')
    parser.add_argument('--test_img_size', type=int, default=640, help='test image sizes')
    parser.add_argument('--hr_input', default=True,action='store_true', help='high resolution input(1280*720)') #if use SR,please set True
